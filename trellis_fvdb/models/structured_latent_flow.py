...
class SparseResBlock3d(nn.Module):
    def __init__(
        ...
        self.norm1 = LayerNorm(channels, elementwise_affine=True, eps=1e-6)
        self.norm2 = LayerNorm(self.out_channels, elementwise_affine=False, eps=1e-6)
        self.conv1 = fvnn.SparseConv3d(channels, self.out_channels, 3)
        self.conv2 = zero_module(fvnn.SparseConv3d(self.out_channels, self.out_channels, 3))
    ...
    def _updown(self, x: fvnn.VDBTensor, target_tensor: Optional[fvnn.VDBTensor] = None) -> fvnn.VDBTensor:
        if self.updown is not None:
            if self.downsample:
                x = self.updown(x, ref_coarse_data=target_tensor)
            elif self.upsample:
                x = self.updown(x, ref_fine_data=target_tensor)
        return x

    def forward(self, x: fvnn.VDBTensor, emb: torch.Tensor, target_tensor: Optional[fvnn.VDBTensor] = None) -> fvnn.VDBTensor:
        emb_out = self.emb_layers(emb).type(x.dtype)
        scale, shift = torch.chunk(emb_out, 2, dim=1)

        x = self._updown(x, target_tensor)
        # fix bug , must under updown
        batch_idx = x.jidx.long()
...
class SLatFlowModel(nn.Module):
    def __init__(
        ...
        self.input_layer = fvnn.Linear(in_channels, io_block_channels[0])
        ...
        self.out_layer = fvnn.Linear(io_block_channels[0], out_channels)
    ...
    # https://github.com/nv-tlabs/SCube/blob/main/scube/modules/diffusionmodules/openaimodel/unet_sparse_crossattn.py#L38
    def forward(self, x: fvnn.VDBTensor, t: torch.Tensor, cond: torch.Tensor) -> fvnn.VDBTensor:
