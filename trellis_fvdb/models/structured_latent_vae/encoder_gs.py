...
class SLatEncoderGS(SparseTransformerBase):
    def __init__(
        self,
        resolution: int,
        in_channels: int,
        ...
        # add two layer
        self.in_layer = fvnn.Linear(in_channels // 2, in_channels)
        self.act_layer = fvnn.GELU()

        self.resolution = resolution
        self.out_layer2 = fvnn.Linear(model_channels, latent_channels)
    ...
    def initialize_weights(self) -> None:
        ...
        torch.nn.init.xavier_uniform_(self.out_layer2.weight)
    ...
    def forward(self, x: fvnn.VDBTensor):
        ...
        x = fvnn.VDBTensor(x.grid, x.data.jagged_like(F.layer_norm(x.data.jdata.to(x.data.jdata.dtype), x.data.jdata.shape[-1:])), x.kmap)
        x = self.in_layer(x)
        x = self.act_layer(x)
        h = super().forward(x)
